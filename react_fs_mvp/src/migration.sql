/*This page is for data definition, which means creating a table. 
I use PostgreSQL in this application for building the database.*/ 
...
5.  IMPORTANT! The next step is db migration (inserting the dable into the database). 
    ...
    your migration file exists; use commands cd and ls to find the file.
...
8.  Verify that the desired table attributes (columns) are contained in the table. 
    You should alread be in psql. Type in the command '\d' followed by the table name(s).
